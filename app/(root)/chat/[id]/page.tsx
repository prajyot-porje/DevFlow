...
import { buildCodePrompt, greetingMessage, quickPrompts } from "@/data/data"
...
function convertToWebContainerFileSystem(fileStructure: InputFileStructure): FileSystemTree {
...
export default function ChatWorkspacePage() {
  ...
  const [title, setTitle] = useState("DevFlow AI Code Studio")
  ...
  const getData = async () => {
    ...
    // Set title from info if available
  ...
  const updateWorkspaceInfoIfNeeded = async (newTitle?: string, newDesc?: string) => {
    if (!id) return
  ...
  const GetAiResponse = async () => {
    ...
    const pro = JSON.stringify(message) + (ChatPrompt.CHAT_PROMPT || "")
    const response = await axios.post("/api/AI_chat", {
      prompt: pro,
    })
    setTitle(response.data.result.title)
    await updateWorkspaceInfoIfNeeded(response.data.result.title)
  ...
  const GenerateAiCode = async (aiContent: string) => {
    ...
    const mergefiles = { ...files, ...code_response }
    setFiles(mergefiles)
    await UpdateFiles({
      workspaceID: id as Id<"workspaces">,
      files: mergefiles,
